...
struct GPSStaircaseDetailView: View {
    ...
    @State var verificationResult: VerifyLocationState = .verifing
    ...
    var body: some View {
        ...
        .sheet(isPresented: $isShowingMissionSheet) {
            VStack {
                ...
                if (verificationResult == .verifing) {
                    ProgressView()
                } else if (verificationResult == .denied) { // 위치 인증을 성공하지 못 했을 때
                    Image("ShakeBird")
                        .resizable()
                        .scaledToFit()
