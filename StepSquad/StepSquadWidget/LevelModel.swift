...
func calculateLevel(for flightsClimbed: Int) -> (level: Level, imageName: String)? {
    ...
    for (_, level) in levels {
        if flightsClimbed >= level.minStaircase && flightsClimbed <= level.maxStaircase {
            let difficultyIndex = level.level
            let imageName = "\(level.difficulty.rawValue)\(difficultyIndex)"
